...
var LocationService = class _LocationService {
  ...
  // Export QR codes
  exportQRCodes() {
    return this.http.get(`${environment.apiUrl}/locations/export-qr`, __spreadProps(__spreadValues({}, this.getAuthHeaders()), {
      responseType: "blob"
    }));
  }
  // Get assets currently assigned to a location
  getLocationAssets(locationId, params = {}) {
    let httpParams = new HttpParams();
    Object.keys(params).forEach((key) => {
      const value = params[key];
      if (value !== void 0 && value !== null && value !== "") {
    ...
    return this.http.get(`${this.apiUrl}/${locationId}/assets`, __spreadValues({
      params: httpParams
    }, this.getAuthHeaders()));
  }
  // Get assignable assets for a location
  getAssignableAssets(locationId, params = {}) {
    let httpParams = new HttpParams();
    Object.keys(params).forEach((key) => {
      const value = params[key];
      if (value !== void 0 && value !== null && value !== "") {
    ...
    return this.http.get(`${this.apiUrl}/${locationId}/assignable-assets`, __spreadValues({
      params: httpParams
    }, this.getAuthHeaders()));
  }
  // Assign assets to a location
  assignAssets(locationId, assetIds, departmentId) {
    const payload = {
      asset_ids: assetIds
    };
    if (departmentId) {
...
//# sourceMappingURL=chunk-HLNFKPU6.js.map
